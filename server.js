...
// 🟦 Get payment history for a specific seat
...
app.post("/students", async (req, res) => {
  ...
  if (student) {
    ...
    // Add current month if not already tracked
...
app.post("/students/:seat/send-alert", async (req, res) => {
  ...
  const messageBody =
    customMessage ||
    `Hello ${student.name}, your library fee of ₹${student.fee} is pending. Please pay it soon.`;
...
app.get("/summary/:month", async (req, res) => {
  ...
  students.forEach((student) => {
    ...
    const record = student.paymentHistory?.find((r) => r.month === month);
